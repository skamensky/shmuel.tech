...
func (c *Config) Generate() {
	...
	case "proto":
		for _, targetDirectory := range c.targetDirectroriesQualified() {
			...
			res := command.NewCommand("protoc", args...).WithVerbose().WithTreatStderrAsErr().Run()

			if res.Err != nil {
				panic(res.Err)
			}
			closestModule := path.Dir(findClosestModFile(targetDirWithName))
			res = command.NewCommand("go", "mod", "tidy").WithVerbose().WithTreatStderrAsErr().WithDir(closestModule).Run()
	...
	case "raw_source":
		for _, targetDirectory := range c.targetDirectroriesQualified() {
			...
			res := command.NewCommand("cp", "--recursive", c.sourceQualified(), targetDirectory).WithVerbose().WithTreatStderrAsErr().Run()
			if res.Err != nil {
				panic(res.Err)
			}
		}
